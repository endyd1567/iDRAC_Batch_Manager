...
> Dell PowerEdge 서버의 **iDRAC Service Tag 수집 → Static IP 일괄 변경 → IPMI / 계정 관리**를 웹 브라우저에서 처리하는 자동화 도구
...
Dell iDRAC Manager는 다수의 Dell 서버를 한 번에 설정해야 하는 환경(서버 렉 구성, 데이터센터 초기 세팅 등)에서 수작업을 자동화하기 위해 만든 웹 기반 관리 도구입니다.

서버에 전원을 켜면 DHCP로 임시 IP가 할당됩니다. 이 도구는 해당 대역을 스캔해 각 서버의 Service Tag를 수집하고, 미리 작성해둔 CSV 파일(Service Tag ↔ Static IP 매핑)을 불러와 racadm 명령으로 IP를 일괄 변경합니다. 이후 IPMI Over LAN 활성화, Power Hot Spare 설정, root 계정 변경까지 한 화면에서 처리할 수 있습니다.
...
- 서버 재시작 시 포트 자동 탐색 (8080 → 8888 → 9090 → …)
...
### 환경 설정
![환경설정](screenshots/3_환경설정.png)

### 태그 수집 — 스캔 결과
![스캔결과](screenshots/5_스캔결과2.png)

### Static IP 패널
![StaticIP설정](screenshots/6_StaicIP_설정3.png)

### 전체 IP 변경 완료
![IP변경결과](screenshots/12__IP변경_결과.png)

### iDRAC 관리
![iDRAC관리](screenshots/16-1_IPMI_활성화_결과.png)
...
### STEP 05 — IP 변경 후 설정 업데이트

변경 완료 후 **환경 설정**의 스캔 대역을 새로운 Static IP 대역으로 변경하고 저장합니다.  
이후 재스캔하면 변경된 IP로 서버들이 정상 탐색됩니다.
...
```
idrac-manager/
├── server.py          # FastAPI 백엔드 (REST + SSE), ~600줄
├── index.html         # 단일 파일 웹 UI (Vanilla JS), ~1300줄
├── requirements.txt
├── README.md
└── screenshots/       # 사용 안내 스크린샷
```
